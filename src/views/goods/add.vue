  ...
  methods: {
    ...
    async tabClicked() {
      if (this.activeIndex === "1") {
        ...
        if (res.meta.status !== 200) {
          return this.$message.error("获取动态参数列表失败!" + res.meta.msg);
        }
    ...
    // 处理图片预览效果
    handlePreview(file) {
      this.previcewPath = file.response.data.url
    ...
    addGoods() {
      this.$refs.addFormRef.validate( async valid => {
        if(!valid) {
          return this.$message.error('请镇写必要的表单项!')
        }
        // 这里如果这样写的话 会报错，原因是
        // v-model 中双向绑定的是一个数组。你join 把数组改成了字符串，
        //这里满足了条件但v-model 那里就发生了错误
